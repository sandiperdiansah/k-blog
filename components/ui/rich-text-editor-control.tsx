...
export interface ButtonControlProps extends Omit<IconButtonProps, "aria-label"> {
...
export function createBooleanControl(config: BooleanControlConfig) {
    const { label, icon: Icon, isDisabled, command, getVariant, getProps } = config;
...
export const FontFamily = createSelectControl({
    ...
    getValue: (editor) => editor.getAttributes("textStyle")?.fontFamily || "default",
...
export const AlignLeft = createBooleanControl({
    ...
    getVariant: (editor) => (editor.isActive({ textAlign: "left" }) ? "subtle" : "ghost"),
});
